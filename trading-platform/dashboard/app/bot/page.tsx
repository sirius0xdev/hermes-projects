...
function StatusIndicator({ status }: { status: string }) {
  ...
  return (
    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold border ${
      isRunning ? 'bg-neon-cyan/[0.08] border-neon-cyan/20 text-neon-cyan' : 'bg-neon-pink/[0.08] border-neon-pink/20 text-neon-pink'
    }`}>
      <span className="relative flex h-2 w-2">
        {isRunning && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-neon-cyan opacity-60"></span>}
        <span className={`relative inline-flex rounded-full h-2 w-2 ${isRunning ? 'bg-neon-cyan' : 'bg-neon-pink'}`}></span>
      </span>
      {status.toUpperCase()}
    </div>
  );
...
function SignalPill({ signal }: { signal: Signal }) {
  const color = signal.strength > 0.8 ? 'text-neon-cyan' : signal.strength > 0.6 ? 'text-accent' : 'text-text-dim';
  const typeColors: Record<string, string> = {
    whale: 'bg-neon-pink/[0.08] text-neon-pink',
    trending: 'bg-neon-green/[0.08] text-neon-green',
    polymarket: 'bg-neon-cyan/[0.08] text-neon-cyan',
    launch: 'bg-neon-pink/[0.08] text-neon-pink',
    onchain: 'bg-neon-green/[0.08] text-neon-green',
  };
  return (
    <div className="bg-bg-elevated neon-border-cyan rounded-md p-3 hover:border-neon-cyan/30 transition-colors group">
      ...
      <div className="font-semibold text-text mb-1.5 font-mono">{signal.asset}</div>
    ...
    <div className="card-hover p-4">
      <div className="flex items-center justify-between mb-3">
        <div className={`px-3 py-1 text-xs font-bold rounded-full uppercase tracking-wider ${
          decision.type === 'BUY' ? 'bg-neon-cyan/[0.08] text-neon-cyan' :
          decision.type === 'SELL' ? 'bg-neon-pink/[0.08] text-neon-pink' :
        ...
        <div className="flex items-center gap-2">
          ...
          {decision.executed && (
            <span className="text-[10px] px-2 py-0.5 bg-neon-cyan/[0.1] text-neon-cyan rounded font-semibold">EXECUTED</span>
          )}
        </div>
      </div>

      <div className="flex items-baseline gap-2 mb-3">
        <span className="text-xl font-bold text-text font-mono">{decision.asset}</span>
        <span className="font-mono text-[11px] text-text-secondary">• {decision.sizeSol} SOL</span>
      </div>

      <div className="text-[11px] leading-relaxed text-text-secondary border-l-2 border-neon-cyan/30 pl-3 mb-4">
        {decision.rationale}
      </div>

      <div className="flex items-center justify-between text-[11px]">
        <div className="flex items-center gap-3">
          <div>Confidence <span className="font-mono text-neon-cyan font-semibold">{(decision.confidence * 100).toFixed(0)}%</span></div>
        ...
        {decision.realizedPnl !== undefined && (
          <div className={`font-mono font-semibold ${decision.realizedPnl >= 0 ? 'text-neon-cyan' : 'text-neon-pink'}`}>
...
function EquityCurve({ data }: { data?: { time: string; value: number }[] }) {
  ...
  const lineColor = isUp ? '#00fff7' : '#ff00ff';
  ...
  return (
    <div className="relative">
      ...
      <div className="flex justify-between text-[11px] text-text-dim px-1 mt-1">
        <div className="font-mono">{chartData[0].time}</div>
        <div className={`font-mono font-semibold ${isUp ? 'text-neon-cyan' : 'text-neon-pink'}`}>
          {isUp ? '+' : ''}{((chartData[chartData.length-1].value - chartData[0].value) / chartData[0].value * 100).toFixed(1)}%
        </div>
        <div className="font-mono">{chartData[chartData.length-1].time}</div>
      </div>
    </div>
  );
}
      ...
      <AppShell>
        <div className="flex h-96 items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="text-text-dim text-sm font-mono">Connecting to autonomous quant loop...</div>
          </div>
        </div>
      </AppShell>
    );
      ...
      <div className="space-y-6">
          ...
          <div className="xl:col-span-7 space-y-5">
            ...
            <div className="card-hover p-5">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <h3 className="font-semibold text-text font-mono tracking-tight">Equity Curve (30d)</h3>
                  <p className="text-[11px] text-text-dim mt-0.5 font-mono">Daily P&amp;L focus • Kelly sizing active</p>
                </div>
                <div className="text-right">
                  <div className="text-[10px] text-text-dim uppercase tracking-wider font-semibold font-mono">Total PnL</div>
                  <div className="text-xl font-bold text-neon-cyan font-mono">+${botStatus.totalPnl.toFixed(1)}</div>
                </div>
              </div>
              <EquityCurve />
            </div>
            ...
            {/* Strategy Metrics */}
            <div className="card-hover p-5">
              <h3 className="text-[10px] uppercase tracking-widest text-text-dim font-semibold mb-4 flex items-center gap-2 font-mono">
                <Activity className="w-3.5 h-3.5" />
                Strategy Metrics
                <span className="text-[10px] px-2 py-0.5 bg-neon-cyan/[0.08] text-neon-cyan rounded font-semibold">v0.4</span>
              </h3>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {metrics.map((m, i) => (
                  <div key={i} className="bg-bg-elevated neon-border-cyan rounded-md p-4 hover:border-neon-cyan/30 transition-colors">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="text-[11px] text-text-dim font-mono uppercase tracking-wider">{m.name}</div>
                      ...
                      <div className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${
                        m.change24h >= 0 ? 'bg-neon-cyan/[0.08] text-neon-cyan' : 'bg-neon-pink/[0.08] text-neon-pink'
                      }`}>
                        {m.change24h >= 0 ? <ArrowUpRight className="w-3 h-3 inline" /> : <ArrowDownRight className="w-3 h-3 inline" />}
                        {Math.abs(m.change24h)}
                      </div>
                    </div>
                    <div className="text-[10px] text-text-dim mt-3 font-mono">{m.description}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="xl:col-span-5 space-y-5">
            ...
            <div className="card-hover p-5">
              <div className="flex justify-between mb-5">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-md bg-neon-green/[0.08] flex items-center justify-center neon-border-cyan">
                    <Zap className="w-4 h-4 text-neon-green" />
                  </div>
                  <div>
                    <div className="font-semibold text-text font-mono">Live Decision Engine</div>
                    <div className="text-[10px] text-text-dim font-mono">Kafka • Postgres • Jupiter</div>
                  </div>
                </div>
                <div className="text-right text-[10px]">
                  <div className="text-text-dim uppercase tracking-wider font-semibold font-mono">Signals</div>
                  <div className="text-2xl font-mono text-neon-cyan font-bold">{botStatus.signalsProcessed}</div>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="bg-bg-elevated neon-border-cyan rounded-md p-3">
                  <div className="text-text-dim text-[10px] mb-1 uppercase tracking-wider font-semibold font-mono">Uptime</div>
                  <div className="font-mono font-semibold">{botStatus.uptime}</div>
                </div>
                <div className="bg-bg-elevated neon-border-cyan rounded-md p-3">
                  <div className="text-text-dim text-[10px] mb-1 uppercase tracking-wider font-semibold font-mono">Last Decision</div>
              ...
              <div className="mt-4 pt-4 border-t border-bg-border text-[11px] font-mono text-text-dim">
                Position: <span className="text-neon-cyan font-semibold">{botStatus.currentPosition}</span>
              </div>
            </div>
            ...
            <div className="card-hover p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xs font-semibold text-text font-mono uppercase tracking-wider">Live Signal Feed</h3>
                <div className="text-[10px] bg-neon-cyan/[0.08] px-2.5 py-1 rounded font-mono text-neon-cyan font-semibold uppercase tracking-wider">Real-time</div>
              </div>
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div>
              <h4 className="text-[10px] uppercase tracking-widest text-text-dim mb-3 pl-1 font-semibold font-mono">Live Decisions (Last 6)</h4>
              <div className="space-y-3">
                {decisions.slice(0, 3).map(d => <DecisionCard key={d.id} decision={d} />)}
              </div>
            </div>
            <div>
              <h4 className="text-[10px] uppercase tracking-widest text-text-dim mb-3 pl-1 font-semibold font-mono">Backtest Summary</h4>
              <div className="card-hover p-5">
                {backtests.slice(0, 2).map((bt, idx) => (
                  <div key={bt.id} className={`${idx !== 0 ? 'mt-6 pt-6 border-t border-bg-border' : ''}`}>
                    ...
                    <div className="grid grid-cols-4 gap-3 mt-4 text-xs">
                      ...
                      <div>
                        <div className="text-text-dim text-[10px] uppercase tracking-wider font-semibold font-mono">Return</div>
                        <div className="font-mono text-lg text-text font-bold">+{(bt.totalReturn * 100).toFixed(0)}%</div>
                      </div>
                      <div>
                        <div className="text-text-dim text-[10px] uppercase tracking-wider font-semibold font-mono">Max DD</div>
                        <div className="font-mono text-lg text-neon-pink font-bold">{(bt.maxDrawdown * 100).toFixed(1)}%</div>
                      </div>
                      <div>
                        <div className="text-text-dim text-[10px] uppercase tracking-wider font-semibold font-mono">Trades</div>
        ...
        {activeTab === 'decisions' && (
          <div className="space-y-3">
            ...
            <div className="text-center text-[11px] text-text-dim py-8 border border-dashed border-bg-border rounded-xl font-mono">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-bg-border text-left text-[10px] text-text-dim uppercase tracking-wider bg-bg-elevated/50">
                  ...
                  <th className="px-5 py-3.5 font-semibold font-mono">Sharpe</th>
                  <th className="px-5 py-3.5 font-semibold font-mono text-right">Win Rate</th>
                  <th className="px-5 py-3.5 font-semibold font-mono text-right">Max DD</th>
                  <th className="px-5 py-3.5 font-semibold font-mono text-right">Return</th>
                  <th className="px-5 py-3.5 font-semibold font-mono text-right">Trades</th>
                  <th className="px-5 py-3.5 font-semibold font-mono">Period</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-bg-border/50 font-mono text-sm">
                {backtests.map(bt => (
                  <tr key={bt.id} className="hover:bg-bg-hover transition-colors">
                    ...
                    <td className="px-5 py-4 text-neon-cyan font-semibold">{bt.sharpeRatio.toFixed(2)}</td>
                    <td className="px-5 py-4 text-right text-neon-cyan font-semibold">{(bt.winRate * 100).toFixed(0)}%</td>
                    <td className="px-5 py-4 text-right text-neon-pink font-semibold">{(bt.maxDrawdown * 100).toFixed(1)}%</td>
                    <td className="px-5 py-4 text-right text-neon-cyan font-semibold">+{(bt.totalReturn * 100).toFixed(0)}%</td>
